defmodule GRPC.Client.Adapters.MintV2 do
  @moduledoc """
  A client adapter using mint
  """

  alias GRPC.Channel
  alias GRPC.Client.Adapters.Mint.ConnectionProcessV2
  ...
  def connect(%{host: host, port: port} = channel, opts \\ []) do
    ...
    channel
    |> mint_scheme()
    |> ConnectionProcessV2.start_link(host, port, opts)
    |> case do
      {:ok, pid} ->
  ...
  def disconnect(%{adapter_payload: %{conn_pid: pid}} = channel)
      when is_pid(pid) do
    :ok = ConnectionProcessV2.disconnect(pid)
  ...
  def send_data(
    ...
    {:ok, data, _} = GRPC.Message.to_data(message, opts)
    :ok = ConnectionProcessV2.stream_request_body(pid, request_ref, data)
    if opts[:send_end_stream], do: ConnectionProcessV2.stream_request_body(pid, request_ref, :eof)
    stream
  end
  ...
  def end_stream(
    ...
    ConnectionProcessV2.stream_request_body(pid, request_ref, :eof)
    stream
  end
  ...
  def cancel(stream) do
    ...
    ConnectionProcessV2.cancel(conn_pid, request_ref)
  end
  ...
  defp do_request(
    ...
    response =
      ConnectionProcessV2.request(pid, path, headers, body,
